...
/*excluded:**/
    ...
    function nodeJSExports(defaultPrefix){
        ...
        function setupBrowserFiles(app,public_path,console){
            ...
            var minified_len,browser_len = self_serve.length;
          
            var browser_min_sha1,browser_sha1 = sha1Hash (self_serve);
            var browser_sha1_file = tab_calls_browser_filename+".sha1";
            var browser_min_sha1_file = tab_calls_browser_min_filename+".sha1";
          
            
            
            
          
            var uglifyNeeded = true; 
          
            if (  fs.existsSync(browser_sha1_file) && 
                  fs.existsSync(browser_min_sha1_file) && 
                  fs.existsSync(tab_calls_browser_filename)  &&
                  fs.existsSync(tab_calls_browser_min_filename)  
               ) {
                uglifyNeeded = fs.readFileSync(browser_sha1_file,"utf8")!==browser_sha1;
                
                if (!uglifyNeeded)  {
                  // check the stored unminified file matches the one in memory, if not, redo the minify anyway
                  if(console)   
                     console.log("hashing stored "+tab_calls_browser_filename);
                  uglifyNeeded = sha1Hash(fs.readFileSync(tab_calls_browser_filename,"utf8"))!==browser_sha1;
                }
                  
                 
               
                if (!uglifyNeeded)  {
                  // check the stored minified file matches the previously hashed one
                  if(console)   
                     console.log("hashing stored minified "+tab_calls_browser_min_filename);
                  var temp = fs.readFileSync(tab_calls_browser_min_filename,"utf8");
                  minified_len = temp.length;
                  browser_min_sha1 = sha1Hash(temp);
                  uglifyNeeded = fs.readFileSync(browser_min_sha1_file,"utf8")!==browser_min_sha1;
                }
              }
            ...
            if (uglifyNeeded) {
                if(console)   
                  console.log("saving updated..."+tab_calls_browser_filename);
              
                fs.writeFileSync(tab_calls_browser_filename,self_serve);
                fs.writeFileSync(browser_sha1_file,browser_sha1);
              
                if(console)   
                  console.log("minifying..."+tab_calls_browser_min_filename);
                ...
                minified_len = self_serve.code.length;
                fs.writeFileSync(tab_calls_browser_min_filename,self_serve.code);
                browser_min_sha1 = sha1Hash(self_serve.code);
                fs.writeFileSync(browser_min_sha1_file,browser_min_sha1);
            ...
            if(console) { 
                  console.log({
                      UglifyJS: {
                          ...
                          sha1: {
                            browser  : browser_sha1,
                            browser_minified : browser_min_sha1,
                          }
